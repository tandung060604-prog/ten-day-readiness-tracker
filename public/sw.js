const CACHE_NAME = 'little-days-shell-v1'
...
self.addEventListener('fetch', event => {
  ...
  event.respondWith(
    isNavigation
      ? fetch(event.request).then(response => { const copy = response.clone(); caches.open(CACHE_NAME).then(cache => cache.put('/index.html', copy)); return response }).catch(() => caches.match('/index.html'))
