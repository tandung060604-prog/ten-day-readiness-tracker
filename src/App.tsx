...
function InsightsView({logs,settings}:{logs:DailyLog[];settings:AppSettings}) {
  const scores=logs.map(l=>readiness(l,settings.waterTargetMl)); const sleep=logs.map(l=>({label:`D${l.dayNumber}`,value:l.sleep?.nightHours||0,max:9})); const water=logs.map(l=>({label:`D${l.dayNumber}`,value:Math.round(l.hydrationMl/100)/10,max:3}));
  return <><div className="grid-3"><section className="metric-card"><small>AVG READINESS</small><h3>{Math.round(scores.reduce((a,b)=>a+b,0)/scores.length)}<span>/100</span></h3><p>Personal adherence score</p></section><section className="metric-card"><small>WORKOUTS</small><h3>{logs.filter(l=>l.workout?.completed).length}<span>/10</span></h3><p>Completed sessions</p></section><section className="metric-card"><small>8H SLEEP DAYS</small><h3>{logs.filter(l=>(l.sleep?.nightHours||0)>=8).length}<span>days</span></h3><p>Consistency matters</p></section></div><div className="grid-2"><section className="card"><div className="section-head"><div><small>TREND</small><h3>Sleep hours</h3></div></div><BarChart data={sleep}/></section><section className="card"><div className="section-head"><div><small>TREND</small><h3>Hydration · L</h3></div></div><BarChart data={water}/></section></div><section className="card disclaimer">This dashboard is a personal adherence and wellness tracker, not a medical assessment.</section></>
}
